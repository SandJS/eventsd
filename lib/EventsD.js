...
class EventsD extends SandGrain {
   ...
   */
  sendMetric(name, value, meta = {}, time = (new Date()).toISOString(), extra = {}) {
    return runWithCallbackOrPromise(function(cb) {
      let msg = {};

      let extraMeta = _.isFunction(this.config.metrics.extraMeta) ? this.config.metrics.extraMeta : () => {};
      ...
      if (_.isPlainObject(name)) {
        ...
        msg = _.merge({
          name: name,
          value: value,
          meta: _.merge({}, extraMeta(name), meta),
          time: time,
          nanoseconds: nano()
        }, extra || {});
      }
